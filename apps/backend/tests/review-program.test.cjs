...
test("no code inside the cooldown, below the rating, without an email or twice for one review", async () => {
  ...
  assert.equal(noEmail.updates[0].reward_note, "no email or mobile to send it to")
  const placeholder = rewardHarness({ review: { ...base, email: "01712345678@no-email.florayn.local" } })
  assert.equal(await placeholder.run(), null, "the checkout's placeholder address is not an email")
  assert.equal(placeholder.mails.length, 0)
