...
const TABLE = "checkout_setting"
const REQUIRED_COLUMNS = ["id", "heading", "description", "delivery_note", "support_phone",
  "support_label", "show_order_note", "created_at", "updated_at", "deleted_at"]

/**
 * Preflight: medusa exec ./src/scripts/migrate-checkout-settings.ts
 * Apply: medusa exec ./src/scripts/migrate-checkout-settings.ts apply Migration20260920194733
 *
 ...
 */
export default async function migrateCheckoutSettings({ container, args = [] }: ExecArgs) {
  ...
  if (database.pathname !== "/florayn_v3" && !/^\/florayn_checkout_test_[a-z0-9_]+$/.test(database.pathname)) {
    throw new MedusaError(MedusaError.Types.NOT_ALLOWED, "This migration is restricted to the new-site Medusa database or a disposable checkout test database")
  }
    ...
    try {
      ...
      return { migration: MIGRATION, recorded: names.includes(MIGRATION),
        ...
        recorded_content_migrations: names.filter((name) => [
          ...
          "Migration20260917210000", MIGRATION,
        ].includes(name)),
  ...
  logger.info(`CHECKOUT_MIGRATION_PREFLIGHT ${JSON.stringify(before)}`)
  if (!apply) return
  await container.resolve(Modules.LOCKING).execute(`florayn-migration:${MIGRATION}`, async () => {
    ...
    if (!after.recorded || missing.length || !after.indexes.includes("IDX_checkout_setting_deleted_at")) {
      throw new MedusaError(MedusaError.Types.DB_ERROR, "Checkout migration schema verification failed")
    }
    logger.info(`CHECKOUT_MIGRATION_VERIFIED ${JSON.stringify(after)}`)
  }, { timeout: 120 })
}
