...
export default function ProductEditor({ slug, onBack, onChanged, onDuplicate, onPricing }: { slug: string; onBack: () => void; onChanged: () => void; onDuplicate: (d: Detail) => void; onPricing: () => void }) {
  ...
  return <div className="grid gap-5">
    ...
    {d.kind === "design" && <Container className="grid gap-4"><Heading level="h2">Add model / case type</Heading><Text size="small">Add one combination with its own gallery. Existing variants keep their IDs and shared stock.</Text><div className="grid gap-3 md:grid-cols-2"><select aria-label="New variant case type" className={selectClass} value={caseSlug} onChange={(e) => setCaseSlug(e.target.value)}><option value="">Choose case type</option>{cases.map((c) => <option key={c.slug} value={c.slug}>{c.name}</option>)}</select><select aria-label="New variant model" className={selectClass} value={deviceSlug} onChange={(e) => setDeviceSlug(e.target.value)}><option value="">Choose model</option>{devices.map((v) => <option key={v.slug} value={v.slug}>{v.name}</option>)}</select></div><Gallery images={images} onChange={setImages} slug={slug} disabled={busy} onBusy={setUploading} /><div><Button onClick={addPair} isLoading={busy} disabled={uploading || !caseSlug || !deviceSlug || !images.length}>Add variant</Button></div><CatalogCreate onCreated={catalog} /></Container>}
