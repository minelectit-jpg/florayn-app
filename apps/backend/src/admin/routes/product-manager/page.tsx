...
import { api, audienceLabel, AUDIENCE_OPTIONS, ManagerSelect, type AudienceTag, type Detail } from "../../components/product-manager/shared"

type Summary = { slug: string; name: string; theme: string | null; thumbnail: string | null; forms: string[]; variantCount: number; status: string; kind: "design" | "regular"; audience?: AudienceTag }
...
const ProductManagerPage = () => {
  ...
  const visible = useMemo(() => designs.filter((d) => (!status || d.status === status) && (!kind || d.kind === kind) && (!audience || (d.audience ?? "both") === audience) && `${d.name} ${d.slug} ${d.theme ?? ""}`.toLowerCase().includes(search.toLowerCase().trim())), [designs, search, status, kind, audience])
  async function bulkStatus(next: string) { await bulkPatch({ status: next }) }
  async function bulkPatch(change: Record<string, string>) {
    if (lock.current) return
    lock.current = true; setBusy(true)
    let saved = 0
    try {
      for (const id of selected) {
        await api(`/admin/designs/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(change) })
  ...
  return <div className="grid gap-5">
    ...
    {mode === "bulk" ? <NewDesignPage /> : mode === "pricing" ? <CaseTypesPage /> : <Container className="grid gap-4">
      <div className="grid gap-3 md:grid-cols-[2fr_1fr_1fr_1fr]"><Input aria-label="Search products" placeholder="Search name, URL or collection" value={search} onChange={(e) => { setSearch(e.target.value); setPage(0) }} /><ManagerSelect aria-label="Filter status" value={status} onValueChange={(value) => { setStatus(value); setPage(0) }}><option value="">All statuses</option><option value="published">Published</option><option value="draft">Draft</option><option value="mixed">Mixed</option></ManagerSelect><ManagerSelect aria-label="Filter product type" value={kind} onValueChange={(value) => { setKind(value); setPage(0) }}><option value="">All product types</option><option value="design">Case / design</option><option value="regular">Regular product</option></ManagerSelect><ManagerSelect aria-label="Filter shown for" value={audience} onValueChange={(value) => { setAudience(value); setPage(0) }}><option value="">Women and Men sites</option>{AUDIENCE_OPTIONS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</ManagerSelect></div>
      {selected.length > 0 && <div className="flex flex-wrap items-center gap-2 rounded-lg bg-ui-bg-subtle p-3"><Text size="small">{selected.length} selected</Text><Button size="small" disabled={busy} onClick={() => bulkStatus("published")}>Publish selected</Button><Button size="small" variant="secondary" disabled={busy} onClick={() => bulkStatus("draft")}>Move to draft</Button><span className="mx-1 h-5 w-px bg-ui-border-base" aria-hidden="true" /><Text size="small">Shown for:</Text>{AUDIENCE_OPTIONS.map((o) => <Button key={o.value} size="small" variant="secondary" disabled={busy} onClick={() => bulkPatch({ audience: o.value })}>{o.label}</Button>)}<Button size="small" variant="transparent" disabled={busy} onClick={() => setSelected([])}>Clear selection</Button></div>}
      {error ? <div role="alert"><Text className="text-ui-fg-error">{error}</Text><Button variant="secondary" onClick={load}>Retry</Button></div> : loading ? <Text>Loading products…</Text> : <>
        ...
        <ul className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{visible.slice(page * 24, page * 24 + 24).map((d) => <li key={d.slug} className="overflow-hidden rounded-xl border border-ui-border-base">
          <button type="button" className="block w-full text-left" onClick={() => setParams({ slug: d.slug })}><div className="aspect-square bg-ui-bg-subtle">{d.thumbnail && <img src={d.thumbnail} alt={d.name} loading="lazy" className="size-full object-contain" />}</div><div className="grid gap-2 p-3"><span className="font-medium">{d.name}</span><span className="text-xs text-ui-fg-muted">{d.kind === "regular" ? "Regular product" : d.forms.join(" · ")} · {d.variantCount} variants</span><span className="text-xs">{audienceLabel(d.audience)}</span></div></button>
