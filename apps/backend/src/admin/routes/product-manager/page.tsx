...
import { api, selectClass, type Detail } from "../../components/product-manager/shared"
...
const ProductManagerPage = () => {
  ...
  return <div className="grid gap-5">
    ...
    {mode === "bulk" ? <NewDesignPage /> : mode === "pricing" ? <CaseTypesPage /> : <Container className="grid gap-4">
      <div className="grid gap-3 md:grid-cols-[2fr_1fr_1fr]"><Input aria-label="Search products" placeholder="Search name, URL or collection" value={search} onChange={(e) => { setSearch(e.target.value); setPage(0) }} /><select aria-label="Filter status" className={selectClass} value={status} onChange={(e) => { setStatus(e.target.value); setPage(0) }}><option value="">All statuses</option><option value="published">Published</option><option value="draft">Draft</option><option value="mixed">Mixed</option></select><select aria-label="Filter product type" className={selectClass} value={kind} onChange={(e) => { setKind(e.target.value); setPage(0) }}><option value="">All product types</option><option value="design">Case / design</option><option value="regular">Regular product</option></select></div>
