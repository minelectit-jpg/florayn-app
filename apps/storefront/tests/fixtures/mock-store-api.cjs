...
const caseTypes = [
  { id: "case_signature", name: "Signature", slug: "signature", price: 1400, is_active: true, description: "Test signature case" },
  { id: "case_armor", name: "Armor Black", slug: "armor-black", price: 1700, is_active: true, description: "Test armor case" },
]
...
function makeProduct(slug, title, form = "phone") {
  ...
  const options = [
    { id: `opt_case_${handle}`, title: "Case Type", values: caseTypes.map((c) => ({ id: c.id, value: c.name })) },
    { id: `opt_device_${handle}`, title: "Device", values: productDevices.map((d) => ({ id: d.id, value: d.name })) },
  ]
  const variants = caseTypes.flatMap((c) => productDevices.map((d) => ({
...
const stock = Object.fromEntries(caseTypes.flatMap((c) => devices.map((d) => [`${c.name}|${d.name}`, 20])))
const carts = new Map()
const metrics = []
let revision = 0
let stockRevision = 0
