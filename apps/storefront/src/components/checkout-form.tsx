    ...
    <form ref={formRef} onSubmit={onSubmit} noValidate className="checkout-form" aria-busy={submitting}>
      ...
      <aside className="checkout-aside" aria-label="Review your order and payment">
        ...
        <section className="checkout-payment" aria-labelledby="payment-title">
          ...
          <div className="checkout-grand-total" aria-live="polite" aria-atomic="true"><div><span>{currentQuote ? "Total to pay" : discount > 0 ? "Subtotal after savings" : "Subtotal"}</span><small>{currentQuote ? "Pay in cash on delivery" : "Delivery is added once you choose your district"}</small></div><strong>{payable}</strong></div>
