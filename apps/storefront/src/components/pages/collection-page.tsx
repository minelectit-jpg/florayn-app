...
import { forAudience, withAudience, type Audience } from "@/lib/audience"
...
export async function collectionMetadata({ params }: CollectionRouteParams, audience: Audience): Promise<Metadata> {
  ...
  return {
    title: audience === "men" ? `${group.title} for Men` : group.title,
    description: group.description ?? undefined,
    alternates: { canonical: withAudience(`/collection/${slug}/`, audience) },
  }
}
...
/**
 * One collection in one mode. Men lists only the designs for men (and those
 * for both); florayn.com's old ?filter_gender=men links land on /men/.
 */
export default async function CollectionPage({ params, searchParams, audience }: CollectionRouteParams & { audience: Audience }) {
  const { slug } = await params
  const query = await searchParams
  if (audience === "women" && first(query.filter_gender) === "men") redirect(`/men/collection/${slug}/`)
  ...
  const [{ products: everything }, deviceCatalog, landing] = await Promise.all([
    listProducts({
  ...
  const products = forAudience(everything, audience)
  ...
  return (
    <CollectionShell theme={landing?.theme ?? null}>
      ...
      ) : !products.length && everything.length ? (
        <p className="py-10 text-sm text-ink-muted">
          This collection has no designs for {audience} yet.{" "}
          <Link href="/collections/" className="underline underline-offset-4 hover:text-purple">
