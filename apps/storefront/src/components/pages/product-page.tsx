...
import { fitsAudience, forAudience, readAudienceTag, withAudience, type Audience } from "@/lib/audience"
...
/*
 * The product page in either mode. The route files (app/product/[slug] and
 * app/men/product/[slug]) only fix the mode and the cache settings: generated
 * on first hit, then ISR. A product is the same product in both modes and its
 * canonical URL is always the root one; the mode only changes which designs the
 * "More designs" and recommendation strips offer, and a simple product's colours.
 */
...
export async function productMetadata({ params }: ProductRouteParams): Promise<Metadata> {
...
export default async function ProductPage({ params, audience }: ProductRouteParams & { audience: Audience }) {
  const { slug } = await params
  const pagePath = withAudience(`/product/${slug}/`, audience)
  ...
  const relatedPromise = productPromise.then(async (resolved) => {
    ...
    return Promise.all([
      collectionId
        ...
        const handles = catalog.filter((d) => d.forms.includes(form) && d.slug !== designSlug && fitsAudience(d.audience, audience))
  ...
  const picksPromise = Promise.all([sectionsPromise, productPromise]).then(async ([{ featuredPicks }, resolved]) => {
    ...
    const byHandle = new Map(forAudience(products, audience).map((p) => [p.handle, p]))
    return handles
  ...
  if (!(matrix.caseTypes.length && matrix.devices.length)) {
    ...
    // A colour tagged for the other mode is left out, unless that would leave none.
    const inMode = (product.variants ?? []).filter((v) => fitsAudience(readAudienceTag(v.metadata), audience))
    if (inMode.length) product.variants = inMode
  ...
  const sameFormDesigns = forAudience(pool, audience).filter((p) => productForm(p) === form && p.metadata?.design_slug !== designSlug)
