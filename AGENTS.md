...
.
├── apps/
│   ├── backend/                  # Medusa application (@florayn/backend)
...
**`apps/storefront` is NOT an npm workspace member.** It has its own `node_modules` and its own `package-lock.json`, and it is installed separately. This is deliberate: Medusa's admin peers on React 18 while Next 15's App Router needs React 19, and npm workspaces hoist one React for the whole tree — sharing an install makes Next resolve React 18 and fail the production build with React error #31.

Consequences for any work in this repo:

- Install with `npm run setup` (root install + `npm --prefix apps/storefront install`), never a bare `npm install` alone.
- Add a storefront dependency with `npm --prefix apps/storefront install <pkg>`, or by `cd apps/storefront` first. Adding it from the root installs it into the backend tree instead.
- Turbo only sees the backend. Storefront tasks run through the root's `storefront:*` scripts, which shell out to `npm --prefix apps/storefront`.
- There are two lockfiles by design. Do not "fix" this by folding the storefront back into `workspaces`.
