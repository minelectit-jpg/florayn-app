...
### They share a structure, not a design

Measured at 1440. The ten pages have the same bones - hero, then one or more
sections of heading, copy and a product grid - but were each hand-built, and
their heroes have nothing in common:

| | Leopard | Bug Life |
| --- | --- | --- |
| Hero height | 587px | 792px |
| Heading | 90px / 600 | 136px / 700 |
| Tracking | -3.06px | -4px |
| Alignment | right | left |
| Colour | white | #E8E4D9 |
| Button | white pill, 200x50, r30 | purple pill, 183x52, r999 |
| Button label | Shop Now | Shop Bug Life |

So "match the live ones" cannot mean matching them to each other. What is
shared is the structure and the content; the typography is ten one-off
decisions. The rebuild is one template on the brand scale with the content -
eyebrow, heading, button, copy, hero image, design list - held as data.

Leopard's hero also reads **"Leoapard Collection by Florayn"**, a typo on the
live page. Spelled correctly in the seed.

Section headings split by product form on the live pages - "Leopard Phone
Cases", "Leopard AirPods Case", "Leopard Signature Pen". The Signature Pen has
no equivalent here, and a device is a variant rather than a product in this
build, so the template carries one heading and one grid instead of three.
